...
[子module添加依赖，当前最新版：————> 1.8.1　　　　![最新版](https://img.shields.io/badge/%E6%9C%80%E6%96%B0%E7%89%88-1.8.1-green.svg)](https://search.maven.org/artifact/com.kotlinx/yutils)
```
dependencies {
     //更新地址  https://github.com/yutils/yutils 建议过几天访问看下有没有新版本
     implementation 'com.kotlinx:yutils:1.8.1'
}
```
...
class MyAccountAdapter<T>(context: Context, list: List<T>) : YBaseYRecyclerViewAdapter<T>(context, list) {
    ...
    override fun setViewHolder(itemView: View?): BaseViewHolder {
        return object : BaseViewHolder(itemView) {
            ...
            override fun findView(view: View) { binding = DataBindingUtil.bind(view)!! }
